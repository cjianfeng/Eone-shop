/**
 * Author: SCC
 * 20140416.1 By Ocean 国际化
 * 20140508.1 By Ocean 增加表格多选框change事件
 * Versions: 1.0
 */
(function ($) {
	...
	MMGrid.prototype = {
		...
		_initLayout : function ($el) {
			...
			if (opts.indexCol) {
				opts.cols.unshift({
					title : context.lang.grid.indexText,
		...
		_initOptions : function () {
			...
			if (cols) {
				var bbHtml = ['<h1>' + context.lang.grid.displayColumn + '</h1>'];
				for (var colIndex = 0; colIndex < cols.length; colIndex++) {
					bbHtml.push('<label ');
					...
					bbHtml.push('/><span title="' + col.title + '">');
					if (col.title) {
						bbHtml.push(col.title);
					} else {
						bbHtml.push(context.lang.grid.unDefineTitle);
					}
		...
		_populate : function (items) {
			...
			if (items && items.length !== 0 && opts.cols) {
				...
				for (var rowIndex = 0; rowIndex < items.length; rowIndex++) {
					$.data($trs.eq(rowIndex)[0], 'item', items[rowIndex]);
					$trs.eq(rowIndex).on("dblclick", {
						item : items[rowIndex],
						rowIndex : rowIndex
					}, function (event) {
						that.$body.triggerHandler('itemdblclick', [event.data.item, event.data.rowIndex]);
					});
				}
			} else {
				this._insertEmptyRow();
		...
		_loadAjax : function (args) {
			...
			var params = {};
			//传递参数
			if (typeof args === "string") { //传递
				var arr = args.split("&");
				$.each(arr, function (i, item) {
					var keyvalue = item.split("=");
					params[keyvalue[0]] = decodeURIComponent(keyvalue[1]); //$("#formID").serialize();因为这句话序列化参数时,会成乱码
				});
				this.mmGrid().opts.params = params; //替换参数
				if (this.opts.plugins.length > 0) {
					this.opts.plugins[0].data("page", 1); //替换页数
				}
			} else if (typeof args === "object") { //传递
				params = $.extend(params, args);
				this.mmGrid().opts.params = params; //替换参数
				if (this.opts.plugins.length > 0) {
					this.opts.plugins[0].data("page", 1); //替换页数
				}
			} else if (typeof opts.params === "object") { //初始
				params = opts.params;
			} else if (typeof opts.params === "string") { //初始
				var arr = opts.params.split("&");
				$.each(arr, function (i, item) {
					var keyvalue = item.split("=");
					params[keyvalue[0]] = decodeURIComponent(keyvalue[1]); //$("#formID").serialize();因为这句话序列化参数时,会成乱码
				});
			} else if ($.isFunction(opts.params)) { //初始
			...
			$.ajax({
				...
				if (!$.isPlainObject(obj)) {
					alert(context.lang.grid.ajaxErrMsg1);
					return;
				}
				if (!obj.result) {
					alert(context.lang.grid.ajaxErrMsg2);
					return;
				}
				var list = obj.data[opts.root];
				if (!$.isArray(list)) {
					alert(context.lang.grid.ajaxErrMsg3);
					return;
				}
				that._populate(list);
				if (!opts.remoteSort) {
				...
				that.$body.triggerHandler('loadSuccess', obj);
			}).fail(function (obj) {
				alert(context.lang.grid.ajaxErrMsg4);
				that.$body.triggerHandler('loadError', obj);
			});
		},
		_loadNative : function (args) {
			if ($.isPlainObject(args)) {
				this._populate(args.data.list);
		...
		load : function (args) {
			...
			if ($.isPlainObject(args) && !!args.result) {
				this._loadNative(args);
			} else if ($.isArray(args)) {
				this._loadNative(args);
			} else if (opts.url) {
				this._loadAjax(args); //object({a:1,b:2})或string("a=1&b=2")
			} else if (opts.items) {
			...
			if (typeof args === 'number') {
				...
				if (!opts.multiSelect) {
					...
					if (opts.checkCol) {
						...
						// 20140508.1 By Ocean 增加表格多选框change事件
						$body.find('tr > td').find('.mmg-check').val('off').trigger("change");
					}
				}
				if (!$tr.hasClass('selected')) {
					...
					if (opts.checkCol) {
						...
						// 20140508.1 By Ocean 增加表格多选框change事件
						$tr.find('td .mmg-check').val('on').trigger("change");
					}
				}
					...
					if (args($.data(this, 'item'), index)) {
						...
						if (!$this.hasClass('selected')) {
							...
							if (opts.checkCol) {
								...
								// 20140508.1 By Ocean 增加表格多选框change事件
								$this.find('td .mmg-check').val('on').trigger("change");
							}
						}
					}
				});
				...
				// 20140508.1 By Ocean 增加表格多选框change事件
				$body.find('tr > td').find('.mmg-check').val('on').trigger("change");
			} else {
				return;
			}
		...
		deselect : function (args) { //取消选中
			...
			if (typeof args === 'number') {
				...
				if (opts.checkCol) {
					...
					// 20140508.1 By Ocean 增加表格多选框change事件
					$body.find('tr').eq(args).find('td .mmg-check').val('off').trigger("change");
				}
			} else if (typeof args === 'function') {
				$.each($body.find('tr'), function (index) {
					if (args($.data(this, 'item'), index)) {
						...
						if (opts.checkCol) {
							...
							// 20140508.1 By Ocean 增加表格多选框change事件
							$(this).find('td .mmg-check').val('off').trigger("change");
						}
					}
				});
			} else if (args === undefined || (typeof args === 'string' && args === 'all')) {
		...
		addRow : function (item, index) { //添加数据，第一个参数可以为数组
		...
		updateRow : function (item, index) { //更新行内容，两个参数都必填
			var opts = this.opts;
			var $tbody = this.$body.find('tbody');
			if (!$.isPlainObject(item)) {
				return;
			...
			var that = this;            
			$tr.on("dblclick", {item: item, rowIndex: index}, function(event){                
				that.$body.triggerHandler('itemdblclick', [event.data.item, event.data.rowIndex]);            
			});

		},
		removeRow : function (index) { //删除行，参数可以为索引数组
			var that = this;
	...
	$.fn.mmGrid.defaults = {
		...
		sortName : '',
		sortStatus : 'asc',
		loadingText : context.lang.grid.loadMask,
		noDataText : context.lang.grid.noRecordsFound,
		multiSelect : false,
		checkCol : false,
		indexCol : false,
		indexColWidth : 30,
...
(function ($) {
	...
	MMPaginator.prototype = {
		...
		_initLayout : function () {
			...
			switch (lang) {
			case "en":
				pgHtmls.push('<div class="currentPage"> page <input type="text" /> of <span>1</span></div>');
				break;
			case "cn":
			default:
				pgHtmls.push('<div class="currentPage">当前第<input type="text" />页，共<span>1</span>页</div>');
				break;
			}
			...
			this.$input = $el.find(".currentPage input");
			...
			$limitList.on('change', function () {
				//20140311.1 By Ocean 解决切换每页条数时，不从第一页开始
				$el.data('page', 1);
				that.$mmGrid.load();
			});
			...
			$input.on('keyup', function (e) {
				var value = $input.val();
				$input.val(value.replace(/[^\d]/g, '')); //检验数字
				if (e.keyCode === 13) {
					$el.data('page', value); //todo:检验  小于最大页数,
					that.$mmGrid.load();
				}
			});
		},
		_plain : function (page, totalCount, limit) {
			...
			$.each(list, function (index, item) {
				...
				if (item === '...') {
					...
					$li.text(item).prop('title', that.formatString(context.lang.grid.pageIndex, [item])).on('click', function (e) {
						$el.data('page', item);
						that.$mmGrid.load();
					});
				}
			...
			//
			var $next = $('<li class="next"><a title="' + context.lang.grid.nextPage + '">»</a></li>');
			if (page >= totalPage) {
			...
			$el.find(".currentPage span").html(totalPage);
			$el.find(".currentPage input").val(page);
		},
		_search : function (page, totalCount, limit) {},
		load : function (objPage) {
			var $el = this.$el;
			var $limitList = this.$limitList;
			var opts = this.opts;

			var page = objPage[opts.responseParamName_currentPage];
			if (page === undefined || page === null) {
				page = $el.data('page');
			}
			$el.data('page', page);

			var totalCount = objPage[opts.responseParamName_totalCount];
			if (totalCount === undefined) {
				totalCount = 0;
			}
			$el.data('totalCount', totalCount);

			var limit = objPage[opts.responseParamName_limitCount];
			if (!limit) {
				limit = $limitList.val();
			}
		...
		params : function () {
			...
			params[opts.requestParamName_currentPage] = $el.data('page');
			params[opts.requestParamName_limitCount] = $limitList.val();
			return params;
		},
		init : function ($grid) {
			var that = this;
			...
			this.$mmGrid = $grid;
			this._initLayout();
			this.$mmGrid.on('loadSuccess', function (e, obj) {
				that.load(obj.data.page);
			});
			var params = {};
			params[opts.responseParamName_totalCount] = 0;
			params[opts.responseParamName_currentPage] = opts.page;
			params[opts.responseParamName_limitCount] = opts.limit;
			this.load(params);
			if ($grid.opts.indexCol) {
				...
				indexCol.renderer = function (val, item, rowIndex) {
					...
					(rowIndex + 1 + ((params[opts.requestParamName_currentPage] - 1) * params[opts.requestParamName_limitCount])) +
					'</label>';
				};
			}
	...
	$.fn.mmPaginator.defaults = {
		style : 'plain',
		responseParamName_currentPage : 'currentPage', //返回obj.data.page.currentPage
		responseParamName_totalPage : 'totalPage', //返回obj.data.page.totalPage
		responseParamName_currentCount : 'currentCount', //返回obj.data.page.currentCount
		responseParamName_limitCount : 'limitCount', //返回obj.data.page.responseParamName_limitCount
		responseParamName_totalCount : 'totalCount', //返回obj.data.page.totalCount
		requestParamName_currentPage : 'condition.pageIndex',
		requestParamName_limitCount : 'condition.pageSize',
		limitLabel : '每页{0}条', //每页{0}条
	...
	switch (lang) {
	case "en":
		$.extend($.fn.mmPaginator.defaults, {
			limitLabel : '{0}',
			totalCountLabel : '<span>{0}</span> records'
		});
		break;
	case "cn":
	default:
