    ...
    $(".nav-list").on(ace.click_event, function (g) {
        ...
        if (!f.hasClass("dropdown-toggle")) {
            ...
            //林斌修改开始
                ...
                var li = _loop(jLi);
                var path = $(li).before('<li><i class="icon-home home-icon"></i>首页 <span class="divider"><i class="icon-angle-right arrow-icon"></i></span></li>');
...
function _loop(jLi) {
    jLi.addClass("active");
    var jA = jLi.children("a");
    var name = jA.text();
    var href = jA.attr("href");
    var li = '<li>' + name + '</a><span class="divider"><i class="icon-angle-right arrow-icon"></i></span></li>';
    var jLiP = jLi.parent().closest("li");
    if (jLiP.length == 1) {
        li = _loop(jLiP) + li;
    }
    return li;
...
function general_things() {
    $('.erp-nav [class*="icon-animated-"]').closest("a").on("click", function () {
    ...
    $("#skin-colorpicker").ace_colorpicker().on("change", function () {
        ...
        if (b == "skin-1") {
            $(".erp-nav > li.grey").addClass("dark")
        } else {
            $(".erp-nav > li.grey").removeClass("dark")
        }
        if (b == "skin-2") {
            $(".erp-nav > li").addClass("no-border margin-1");
            $(".erp-nav > li:not(:last-child)").addClass("light-pink").find('> a > [class*="icon-"]').addClass("pink").end().eq(0).find(".badge").addClass("badge-warning")
        } else {
            $(".erp-nav > li").removeClass("no-border margin-1");
            $(".erp-nav > li:not(:last-child)").removeClass("light-pink").find('> a > [class*="icon-"]').removeClass("pink").end().eq(0).find(".badge").removeClass("badge-warning")
        }
        if (b == "skin-3") {
            $(".erp-nav > li.grey").addClass("red").find(".badge").addClass("badge-yellow")
        } else {
            $(".erp-nav > li.grey").removeClass("red").find(".badge").removeClass("badge-yellow")
        }
    })
}
